...
import { ArrowRight, Shield, AlertCircle, BookOpen, Brain, AlertTriangle, Users, Radio, Lock, ShieldAlert, ShieldCheck, Settings2 } from "lucide-react";
						...
						<div className="relative space-y-4">
							<div className="space-y-2">
								...
								<p className="text-zinc-800 dark:text-zinc-300 tracking-tight text-2xl md:text-3xl text-pretty">
									Essential disaster education, safety quizzes, and real time alerts.
								</p>
							</div>
								...
								<GradientBG className="w-full flex items-center justify-between gap-2">
									<div className="w-full flex flex-col min-[350px]:flex-row min-[350px]:items-center gap-0.5 min-[350px]:gap-2 min-w-0">
										<p className="text-xs sm:text-sm font-mono select-none tracking-tighter space-x-1 shrink-0">
											<span>
												<span className="text-red-400">Be Prepared.
													Stay Safe.</span>
											</span>
										</p>
									</div>
...
const tabs: { name: "security" | "features"; content: { icon: React.ReactNode; text: string }[] }[] = [
	{
		name: "security",
		content: [
			{ icon: <Shield className="w-4 h-4 inline mr-2" />, text: "Secure & Responsible Access" },
			{ icon: <Lock className="w-4 h-4 inline mr-2" />, text: "Verified user access only" },
			{ icon: <ShieldAlert className="w-4 h-4 inline mr-2" />, text: "Protected emergency features" },
			{ icon: <ShieldCheck className="w-4 h-4 inline mr-2" />, text: "Safe learning environment" },
			{ icon: <Settings2 className="w-4 h-4 inline mr-2" />, text: "Responsible usage controls" },
		]
	},
	{
		name: "features",
		content: [
			...
			{ icon: <Radio className="w-4 h-4 inline mr-2" />, text: "Real-Time Alerts" },
			{ icon: <Users className="w-4 h-4 inline mr-2" />, text: "Community Preparedness" },
		]
	},
];

function TrafficLightsIcon({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
	return (
		<div className={`flex space-x-2 items-center ${className || ''}`} {...props}>
...
function CodePreview() {
	...
	const [currentTab, setCurrentTab] = useState<"security" | "features">("security");
				...
				>
					<div ref={ref}>
						...
						<div className="pl-4 pt-4">
							...
							<div className="flex flex-col items-start px-6 py-4 text-sm w-full">
